...
struct PreferencesView: View {
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            VStack(spacing: 0) {
                // Menüyü aşağı almak için boşluk
                Spacer()
                    .frame(height: 44)
            ...
            // Sol üst kırmızı kapatma butonu
            Button {
                dismiss()
            } label: {
                CloseButton()
                ...
                HStack {
                    ...
                    if updateChecker.isChecking {
                        ...
                        Button(languageService.s("update_now")) {
                            if let url = URL(string: "https://github.com/alinuxpengui/Macabolic/releases/latest") {
    ...
    private var aboutTab: some View {
        ScrollView {
            VStack(spacing: 16) {
                ...
                Text(languageService.s("version") + " 1.0.0")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
